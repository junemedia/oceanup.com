...
class qs_varnish {
	...
	public static function purge_cache($status=false, $post=false) {
		...
		if (is_object($post) && self::$settings['post_cache']) {
			...
			$resp = self::_quick_request($url, 0.1);
			$last_purge = array_merge($last_purge, $resp);
		}

		if (self::$settings['home_cache']) {
			$resp = self::_quick_request(site_url(), 0.1);
			$last_purge = array_merge($last_purge, $resp);
		}
	...
	protected static function _quick_request($url, $timeout=0.1) {
		$list = array();
		foreach (self::$settings['ips'] as $with) {
			$list[] = self::_curl(self::_replace_domain($url, $with), array(), 'PURGE', false, array(
				'Host: '.self::$find,
				'qsssd-iua: 123',
			));
		}
