...
/**
 * Base widget for each of the Disqus Widgets.
 */

if (!class_exists('QSDCW_Widget')):

abstract class QSDCW_Widget extends WP_Widget {
	protected $_base_dir;
	protected $_base_url;
	protected $_cache_dir;
	protected static $__cache_dir;
	...
	protected function _setup_widget($class, $file) {
		...
		$this->_base_dir = QS_Disqus_Comment_Widgets::$plugin_dir;
		$this->_base_url = QS_Disqus_Comment_Widgets::$plugin_url;
		$this->_cache_dir = $this->_get_cache_dir();
		// cache the templates for this widget
		$this->_cache_templates();
	}

	protected function _cache_file_exists($file) {
		return file_exists($this->_cache_dir.$file) && is_writable($this->_cache_dir.$file);
	}

	protected function _get_cache_file_contents($file) {
		$out = '';
		if (file_exists($this->_cache_dir.$file) && is_readable($this->_cache_dir.$file))
			$out = file_get_contents($this->_cache_dir.$file);
		return $out;
	}

	protected function _put_cache_file_contents($file, $content) {
		$dir = dirname($this->_cache_dir.$file);
		if (!file_exists($dir)) @mkdir($dir, 0777, true);
		if ((file_exists($this->_cache_dir.$file) && is_writable($dir)) || is_writable($dir)) file_put_contents($this->_cache_dir.$file, $content);
	}
	...
	private function _get_cache_dir() {
		...
		self::$__cache_dir = $path = trailingslashit($base.'qsda-widget-cache');

		if (!file_exists($path)) @mkdir($path);
		if (!file_exists($path)) add_action('admin_notices', array(__CLASS__, 'cannot_create_cache_dir'));

		return $path;
	}

	protected static function cannot_create_cache_dir() {
		?>
			<div class="error">
				<p>
					Cannot created path <code><?php echo force_balance_tags(self::$__code_dir) ?></code>.
					Make sure that the path has write permissions on the server.
				</p>
			</div>
		<?php
	}
