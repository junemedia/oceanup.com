...
class Genesis_Featured_Post extends WP_Widget {
	...
	function __construct() {
		...
		$this->WP_Widget( 'featured-post', __( 'Quadshot - Featured Posts', 'genesis' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( $featured_posts->have_posts() ) : echo '<div class="featured-posts-list">'; while ( $featured_posts->have_posts() ) : $featured_posts->the_post();
			echo '<div class="post-item ' . implode( ' ', get_post_class() ) . '">';

			if ( ! empty( $instance['show_image'] ) ) {
				if(has_post_thumbnail()) {
					echo '<div class="shadow-wrap '.esc_attr( $instance['image_alignment'] ).'">';
					$thumb_id = get_post_thumbnail_id();
					$attributes = array( 'class' => 'shadow' );
					$imag = wp_get_attachment_image( $thumb_id, $thumb_size_arr, $attributes );

					//&& $image = genesis_get_image( array( 'format' => 'html', 'size' => $instance['image_size'] ) ) )
					printf( '<a href="%s" title="%s" class="shadow %s">%s</a>', get_permalink(), the_title_attribute( 'echo=0' ), '', $imag );
					echo "</div><!-- /shadow -->";
				}
			}
			...
			if ( ! empty( $instance['show_title'] ) )
				printf( '<h2 class="single-title"><a href="%s" title="%s">%s</a></h2>', get_permalink(), the_title_attribute( 'echo=0' ), get_the_title() );
	...
	function form( $instance ) {
		...
		<div class="genesis-widget-column">
			...
			<div class="genesis-widget-column-box genesis-widget-column-box-top">
				...
				<p>
					<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php _e( 'Post type', 'genesis' ); ?>:</label>
					<?php
		...
		( function( $ ) {
		  $( document ).ready( function() {
			  var dw = $("#" + "<?= $this->get_field_id( 'show_content' );?>");
			  var txt = $("#" + "<?= $this->get_field_id( 'content_limit' );?>");
			  if( $(dw).val() != 'content-limit' ) {
				  $( txt ).parent().css('display', 'none');
			  } else {
				  $( txt ).parent().fadeIn('slow');
				  $( txt ).removeAttr('disabled');
			  }
			  $( dw ).change( function() {
				  var dwval = $(this).val();
				  if(dwval != 'content-limit' ) $( txt ).attr("disabled", "disabled").parent().hide();
				  else $( txt ).removeAttr("disabled").parent().fadeIn('slow');
			  });
		  });
		})( jQuery )
		</script>
